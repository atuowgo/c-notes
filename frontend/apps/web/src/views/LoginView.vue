...
<template>
  <div class="auth-page">
    <form class="auth-card" @submit.prevent="submit">
      ...
      <button class="submit" type="submit" :disabled="loading || !username || !password">
        {{ loading ? '登录中…' : '登录' }}
      </button>
      <p v-if="error" class="auth-error">{{ error }}</p>
      <p v-if="isDev" class="auth-hint">演示账号:demo / demo123</p>
...
.auth-hint { margin: 0; color: #999; font-size: 12px; text-align: center; }
